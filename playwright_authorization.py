...
with sync_playwright() as playwright:
    ...
    # Находим поле "Email" и заполняем его используя test_id
    email_input = page.get_by_test_id('login-form-email-input').locator('input')
    email_input.fill("user.name@gmail.com")

    # Находим поле "Password" и заполняем его используя test_id
    password_input = page.get_by_test_id('login-form-password-input').locator('input')
    password_input.fill("password")

    # Находим кнопку "Login" и кликаем на нее используя test_id
    login_button = page.get_by_test_id('login-page-login-button')
    login_button.click()

    # Проверяем, что появилось сообщение об ошибке используя test_id
